...
plt.fill_between(all_files, CIlow_white, CIup_white, alpha=0.2)
plt.fill_between(all_files, CIlow_black, CIup_black, alpha=0.25)
plt.fill_between(all_files, CIlow_draw, CIup_draw, alpha=0.2)
...
binwidth = 10
sns.histplot(
    means_white, kde=True, bins=int(180 / binwidth), ax=ax, color="red"
)
...
def CI_N(n: int, u: np.float32, var: np.float32, alpha: float) -> tuple:
    """
