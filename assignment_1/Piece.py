...
class Piece:
    ...
    def rook(self):
        ...
        validated = self.validate(
            unvalidated, rook_code, False, old_loc=old_loc[0]
        )
    ...
    def bishop(self):
        ...
        unvalidated = np.transpose(np.array([new_x, new_y]))
        validated = self.validate(
            unvalidated, bishop_code, False, old_loc=old_loc[0]
        )
    ...
    def queen(self):
        ...
        for i in range(len(old_loc)):
            ...
            unvalidated = np.transpose(np.array([new_x, new_y]))
            validated = self.validate(
                unvalidated, queen_code, False, old_loc=old_loc[i], col_int=i
            )
    ...
    def pawn(self):
        ...
        if self.player == 1:  # white
            reg_moves = np.add(
                np.ones(np.shape(old_loc)[0]) * -1, old_loc[:, 0]
            )
        else:  # black
        ...
        counter = len(
            validated
        )  # from what index you should add to the new_loc
        ...
        pawn_moves[counter : counter + len(validated), 0:2] = np.repeat(
            old_loc, 2, axis=0
        )
        pawn_moves[counter : counter + len(validated), 2:4] = validated
        pawn_moves[counter : counter + len(validated), -1] = (
            self.pawn_promotion(validated)
        )
    ...
    def validate(self, moves, piece, leap=True, old_loc=None, col_int=0):
        ...
        if piece % 6 == 0:
            col_int = (
                -1
            )  # using this as condiion to not go into the if-loop below
            code = 1
        elif piece >= 7:
            code = piece % 6 - 1
        else:
    ...
    def leap_validate(self, moves):
        ...
        for j in range(int(len(moves) / 4)):
            piece_enc = (
                False  # keep track whether piece was encountered or not
            )
