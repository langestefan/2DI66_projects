...
class SimResults:
    ...
    def __init__(self, nr_queues):
        ...
        self.sumQL2 = np.zeros((self.nrQueues, 1))# total sum of queue lengths squared
        self.sumW = np.zeros((self.nrQueues, 1))  # total sum of waiting times 
        self.sumW2 = np.zeros((self.nrQueues, 1)) # total sum of waiting times squared
        ...
        self.hist_w = np.ones((self.MAX_ENTRIES, self.nrQueues))*-1    # number of registered waiting times                          
    ...
    def registerQueueLength(self, time, ql):
        ...
        if self.nQL >= self.MAX_ENTRIES:
            raise ValueError(
                f"Amount of entries exceeded expected maximum of {self.MAX_ENTRIES}"
            )
    ...
    def __str__(self):
        ...
        for q in range(self.nrQueues):
            s += f"Mean queue {q+1} length: {str(self.getMeanQueueLength())} \n"
            s += f"Std queue {q+1} length: {str(np.sqrt(self.getVarianceQueueLength()))} \n"
            s += f"Mean waiting time of queue {q+1}: {str(self.getMeanWaitingTime())} \n"
            s += f"Std waiting time of queue {q+1}: {str(np.sqrt(self.getVarianceWaitingTime()))} \n"
