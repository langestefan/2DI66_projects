...
from assignment_2.queue import Queue 
...
class SimHistory:
    ...
    def get_statistics_all(self) -> dict:
        ...
        for i, simulation in enumerate(self.sim_runs):
            ...
            statistics['CustomersCanteen_all'].append(simulation.getMeanCustomersCanteen())
            statistics['SojournTimeCustomer_all'].append(simulation.getMeanSojournCustomer())
            statistics['SojournTimeGroup_all'].append(simulation.getMeanSojournGroup())
            
        statistics['QueueLength_mean'] = np.round(np.mean(statistics['QueueLengths_all']), 3)
        statistics['QueueLength_std'] = np.round(np.std(statistics['QueueLengths_all']), 3)
        statistics['WaitingTime_mean'] = np.round(np.mean(statistics['WaitingTime_all']), 3)
        statistics['WaitingTime_std'] = np.round(np.std(statistics['WaitingTime_all']), 3)
        statistics['CustomersCanteen_mean'] = np.round(np.mean(statistics['CustomersCanteen_all']), 3)
        statistics['CustomersCanteen_std'] = np.round(np.std(statistics['CustomersCanteen_all']), 3)
        statistics['SojournTimeCustomer_mean'] = np.round(np.mean(statistics['SojournTimeCustomer_all']), 3)
        statistics['SojournTimeCustomer_std'] = np.round(np.std(statistics['SojournTimeCustomer_all']), 3)
        statistics['SojournTimeGroup_mean'] = np.round(np.mean(statistics['SojournTimeGroup_all']), 3)
        statistics['SojournTimeGorup_std'] = np.round(np.std(statistics['SojournTimeGroup_all']), 3)
        
        statistics['QueueLength_normal_ci_95'] = self.__ci_normal(
            self.nr_simulations, statistics['QueueLength_mean'], 
            statistics['QueueLength_std']**2, 0.95)
        
        statistics['WaitingTime_normal_ci_95'] = self.__ci_normal(
            self.nr_simulations, statistics['WaitingTime_mean'], 
            statistics['WaitingTime_std']**2, 0.95)
        
        statistics['CustomersCanteen_normal_ci_95'] = self.__ci_normal(
            self.nr_simulations, statistics['CustomersCanteen_mean'], 
            statistics['CustomersCanteen_std']**2, 0.95)
        
        statistics['SojournTimeCustomer_normal_ci_95'] = self.__ci_normal(
            self.nr_simulations, statistics['SojournTimeCustomer_mean'], 
            statistics['SojournTimeCustomer_std']**2, 0.95)
        
        statistics['SojournTimeGroup_normal_ci_95'] = self.__ci_normal(
            self.nr_simulations, statistics['SojournTimeGroup_mean'], 
            statistics['SojournTimeGroup_std']**2, 0.95)
    ...
    def get_statistics_separate(self, queue_nr) -> dict:
        ...
        for i, simulation in enumerate(self.sim_runs):
            ...
            statistics['QueueLengths_all'].append(simulation.getMeanQueueLength()[q])
            statistics['WaitingTimes_all'].append(simulation.getMeanWaitingTime()[q])
...
class QueueSimulator(Simulator) :
    ...
    def __init__(
        ...
        self.queues = np.zeros((nrQueues, 1))
    ...
    def _do_one_run(self, T: int) -> SimResults:
        ...
        for q in range(self.nrQueues):
            self.queues[q] = Queue(self.servDist, self.nrServers)
        ...
        while t < T or N > 0:                   # main loop
            ...
            if e.type == Event.ARRIVAL :    # handle an arrival event
                shortest = np.where(lengths == np.amin(t))[0] # get index of shortest queue
                q = random.choice(shortest) # get (random if same length) shortest queue
                ...
                if self.queues[q].get_length() <= self.queues[q].nrServers: # there was a free server
